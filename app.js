const KEY="tradeSaveV4",OLD_KEY="tradeSaveV3",RATE=16100,USC_PER_USD=100,TARGET_RATE=.05;
const $=id=>document.getElementById(id);
let state=JSON.parse(localStorage.getItem(KEY)||"null");
if(!state){
  const old=JSON.parse(localStorage.getItem(OLD_KEY)||localStorage.getItem("tradeSaveV2")||"null");
  state=old||{capital:1006,trading:[{date:"2026-09-14",profit:5.7,note:"Profit awal"}],cash:[],saving:[],savingTarget:2000};
  // V3 menyimpan cashflow/tabungan dalam USC. V4 mengubah keduanya menjadi IDR.
  if(old && localStorage.getItem(OLD_KEY)){
    state.cash=(state.cash||[]).map(x=>({...x,amount:Number(x.amount||0)*161}));
    state.saving=(state.saving||[]).map(x=>({...x,amount:Number(x.amount||0)*161}));
    state.savingTarget=Number(state.savingTarget||0)*161;
  }
...
function go(page){document.querySelectorAll(".nav").forEach(n=>n.classList.toggle("active",n.dataset.page===page));document.querySelectorAll(".page").forEach(p=>p.classList.toggle("active",p.id===page));$("pageTitle").textContent={dashboard:"Dashboard",trading:"Trading Harian",cashflow:"Uang Masuk / Keluar",saving:"Tabungan",monthly:"Rekap Bulanan"}[page]||"Dashboard";window.scrollTo({top:0,behavior:"smooth"})}
...
function openForm(type,index=null){
  ...
  $("amountLabel").textContent=type==="trading"?"Profit / Loss (USC)":"Nominal (IDR)";
  $("formHint").textContent=type==="trading"?"Gunakan angka negatif untuk loss, misalnya -12.50 USC.":type==="cash"?"Masukkan nominal uang dalam Rupiah (IDR).": "Masukkan nominal tabungan dalam Rupiah (IDR).";
...
function render(){
  ...
  $("saveAmount").textContent=rp(s);$("saveTarget").textContent="Target "+rp(state.savingTarget);$("saveGoalLabel").textContent="Target "+rp(state.savingTarget);$("savePct").textContent=sp.toFixed(1)+"%";$("saveBar").style.width=sp+"%";
  ...
  $("qDeposit").textContent=rp(dep);$("qWithdraw").textContent=rp(wd);$("qSaving").textContent=rp(s);
  $("cashSummary").innerHTML=`<div class="quick-list"><div><span>Total masuk</span><b>${rp(dep)}</b></div><div><span>Total keluar</span><b>${rp(wd)}</b></div><div><span>Net cashflow</span><b>${rp(dep-wd)}</b></div></div>`;
...
function renderCash(){
  ...
  $("cashIn").textContent=rp(inn);$("cashOut").textContent=rp(out);$("cashNet").textContent=rp(inn-out);
  $("cashRows").innerHTML=state.cash.map((x,i)=>`<tr><td>${esc(x.date)}</td><td>${esc(x.type)}</td><td>${rp(x.amount)}</td><td>${rp(x.amount)}</td><td>${esc(x.note)}</td><td><button class="row-edit" onclick="editRow('cash',${i})">Edit</button></td></tr>`).join("")||'<tr><td colspan="6">Belum ada transaksi.</td></tr>';
}
function renderSaving(){
  const s=savings(),pct=state.savingTarget?Math.min(100,s/state.savingTarget*100):0;
  $("saveBig").textContent=rp(s);$("saveBigRp").textContent="";$("saveBigBar").style.width=pct+"%";$("saveBigPct").textContent=pct.toFixed(1)+"%";
  $("savingRows").innerHTML=state.saving.map((x,i)=>`<tr><td>${esc(x.date)}</td><td>${rp(x.amount)}</td><td>${esc(x.note)}</td><td><button class="row-edit" onclick="editRow('saving',${i})">Edit</button></td></tr>`).join("")||'<tr><td colspan="4">Belum ada tabungan.</td></tr>';
}
